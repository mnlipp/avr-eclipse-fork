 ...
 * 
 */
public class AVRGDBDebuggerTab extends AbstractLaunchConfigurationTab implements IAVRGDBConstants,
	...
	private Button				fRemoteGDBServer;
	private Composite			fRemoteOptions;
	private Text				fIpAddress;
	...
	public void initializeFrom(ILaunchConfiguration configuration) {
		try {
			...
			boolean useRemoteAttr = configuration.getAttribute(ATTR_USE_REMOTE_GDBSERVER,
					DEFAULT_USE_REMOTE_GDBSERVER);
			fRemoteGDBServer.setSelection(useRemoteAttr);
			setEnabled(fRemoteOptions, useRemoteAttr);

			String ipAddressAttr = configuration.getAttribute(ATTR_IP_ADDRESS, DEFAULT_IP_ADDRESS);
			fIpAddress.setText(ipAddressAttr);

			int portNumberAttr = configuration.getAttribute(ATTR_PORT_NUMBER, DEFAULT_PORT_NUMBER);
			fPortNumber.setText(String.valueOf(portNumberAttr));
	...
	public void performApply(ILaunchConfigurationWorkingCopy configuration) {
		...
		configuration.setAttribute(ATTR_USE_REMOTE_GDBSERVER, fRemoteGDBServer.getSelection());
		configuration.setAttribute(ATTR_IP_ADDRESS, fIpAddress.getText().trim());
		try {
			configuration.setAttribute(ATTR_PORT_NUMBER, Integer.parseInt(fPortNumber.getText()
					.trim()));
		} catch (NumberFormatException e) {
			configuration.setAttribute(ATTR_PORT_NUMBER, 4242);
		}
	}
	...
	public void setDefaults(ILaunchConfigurationWorkingCopy configuration) {
		...
		configuration.setAttribute(ATTR_USE_REMOTE_GDBSERVER, DEFAULT_USE_REMOTE_GDBSERVER);
		configuration.setAttribute(ATTR_IP_ADDRESS, DEFAULT_IP_ADDRESS);
		configuration.setAttribute(ATTR_PORT_NUMBER, DEFAULT_PORT_NUMBER);
	}
	...
	private void createCommandGroup(Composite parent) {
		...
		gd = new GridData(GridData.FILL_HORIZONTAL);
		fAVRGDBCommand.setLayoutData(gd);
	...
	/**
	 * Add the "Remote Target" group to the parent composite.
	 * 
	 * @param parent
	 */
	private void createRemoteGroup(Composite parent) {
		...
		// /////////////////////////////////////////////////////////
		//
		// The "Use remote GDBServer" option
		//
		// /////////////////////////////////////////////////////////

		fRemoteGDBServer = new Button(group, SWT.CHECK);
		fRemoteGDBServer.setText("Use remote GDBServer");
		fRemoteGDBServer
				.setToolTipText("Select this if AVaRICE is running on a separate computer, connected via a TCP network.");
		fRemoteGDBServer.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				Boolean enabled = fRemoteGDBServer.getSelection();
				setEnabled(fRemoteOptions, enabled);
				updateLaunchConfigurationDialog();
			}
		});

		fRemoteOptions = new Composite(group, SWT.NONE);
		fRemoteOptions.setLayout(new GridLayout(2, false));

		// /////////////////////////////////////////////////////////
		//
		// The "Host name or IP address" option
		//
		// /////////////////////////////////////////////////////////

		Label label = new Label(fRemoteOptions, SWT.NONE);
		label.setText("Host name or IP address:");
		fIpAddress = new Text(fRemoteOptions, SWT.BORDER);
		GridData gd = new GridData();
		gd.widthHint = 100;
		fIpAddress.setLayoutData(gd);
		fIpAddress.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				updateLaunchConfigurationDialog();
			}
		});
		...
		label = new Label(fRemoteOptions, SWT.NONE);
		label.setText("Port number:");
		fPortNumber = new Text(fRemoteOptions, SWT.BORDER);
		gd = new GridData();
		gd.widthHint = 100;
		fPortNumber.setLayoutData(gd);
		fPortNumber.addVerifyListener(new VerifyListener() {
			public void verifyText(VerifyEvent e) {
				e.doit = Character.isDigit(e.character) || Character.isISOControl(e.character);
			}
		});
		fPortNumber.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				updateLaunchConfigurationDialog();
			}
		});
