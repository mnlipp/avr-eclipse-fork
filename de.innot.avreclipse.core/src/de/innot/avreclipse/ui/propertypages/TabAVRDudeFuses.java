 ...
 * </ul>
 * The Fuse byte values can either be entered directly, or a fuses file can be
 * selected which provides the fuse bytes.
 * </p>
 * 
...
public class TabAVRDudeFuses extends AbstractAVRDudePropertyTab {
	...
	private final static String TEXT_READDEVICE = "Read from device";
	...
	private Composite[] fFuseByteCompos = new Composite[MAX_FUSES];
	private Text[] fFuseValueTexts = new Text[MAX_FUSES];
	 ...
	 * <p>
	 * Contains a Text control to enter a filename and three buttons to select
	 * the filename from the workplace, the filesystem or from a build variable.
	 * </p>
	 * 
	 * @param parent
	 *            Parent <code>Composite</code>
	 ...
	 * <p>
	 * Contains controls to edit all fuse bytes directly and two buttons to read
	 * the fuse byte values from the programmer and to copy the values from the
	 * fuses file.
	 * </p>
	 * 
	 * @param parent
	 ...
	 * <p>
	 * The editor consists of a Text control to enter the byte value and a Label
	 * below it with the name of the fusebyte.
	 * </p>
	 * <p>
	 * The Text control will only accept (up to) 2 hex digits (converted to
	 * uppercase). The value is stored as an <code>int</code> in the target
	 * properties, with <code>-1</code> representing an empty value.
	 * </p>
	 * <p>
	 * The Editor uses a <code>FillLayout</code> to pack both elements
	 * tightly. It is up to the caller to set the LayoutData for this composite
	 * </p>
	 * 
	 * @param parent
	 *            Parent <code>Composite</code>
	 ...
	 * <p>
	 * When enabling, only those editors are enabled that are actually valid for
	 * the current MCU.
	 * </p>
	 * 
	 * @param enabled
